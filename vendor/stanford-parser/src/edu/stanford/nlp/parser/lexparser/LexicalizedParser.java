...
public class LexicalizedParser implements ViterbiParser, Function<Object,Tree> {
  ...
  public static Pair<List<Tree>,List<Tree>> getAnnotatedBinaryTreebankFromTreebank(Treebank trainTreebank,
    ...
    if (Test.verbose) {
      ...
      System.out.println(trainTreebank.textualSummary(tlp));
    }
  ...
  protected final ParserData getParserDataFromTreebank(Treebank trainTreebank, DiskTreebank secondaryTrainTreebank, double weight, GrammarCompactor compactor) {
    ...
    if (Test.verbose) {
      ...
      System.err.println(trainTreebank.textualSummary(tlp));
      System.err.print("Secondary training ");
      System.err.println(secondaryTrainTreebank.textualSummary(tlp));
    }
  ...
  public double testOnTreebank(Treebank testTreebank) {
    ...
    if (Test.verbose) {
      ...
      System.out.println(testTreebank.textualSummary(tlp));
    }
    if (Test.evalb) {
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("pcfgCatE"))) {
      pcfgCatE = new ConstituentEvalByCat("pcfg Category Eval", tlp);
    }
   ...
   * tokenizer for the language is ignore, and a universal (Unicode)
  ...
  public static void main(String[] args) {
    ...
    boolean readFromStandardIn = false;
    ...
    while (argIndex < args.length && args[argIndex].charAt(0) == '-') {
      if (args[argIndex].equalsIgnoreCase("-train") ||
        ...
        argIndex += 2;
      } else if (args[argIndex].equalsIgnoreCase("-stream")) {
    ...
    if (testTreebank != null) {
      ...
      if (readFromStandardIn) {
        // Read newline-separated sentences from standard in
        System.err.println("Reading sentences from standard input");
        BufferedReader inStream = new BufferedReader(new InputStreamReader(System.in));
        TreePrint tp = Test.treePrint(op.tlpParams);
        String sentence;
        Tree.DISPLAY_SCORES = false;
        try {
          while ((sentence = inStream.readLine()) != null) {
            lp.parse(sentence);
            tp.printTree(lp.getBestParse());
          }
        ...
        System.err.println("Reading sentences from files");
